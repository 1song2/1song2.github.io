...
Small, Medium, Large 옆에 있는 픽셀들은 Portrait 모드 기준 파일의 높이를 뜻합니다. (사실 Landscape 모드의 화면을 녹화해도 결과물은 Portrait으로 나옵니다. 제대로 Landscape으로 보여주기 위해선 결과물을 회전시켜줘야 합니다.) GIF 파일 크기를 최소로 하기 위해선 **Small (320px)**과 **For smaller file size**를 선택해 주면 되겠죠. 하지만 320px는 앱의 동작을 보여주기에 너무 작다는 느낌이 있었습니다.

**Medium (640px)**을 선택하면 되지 않냐고요? 그럼 파일 크기가 문제입니다. 개인적으로는 1 MB를 크게 넘지 않는 선에서 GIF를 저장하고 싶은데 이 방법으로는 2 MB가 훌쩍 넘어가기 부지기수입니다. (물론 영상의 길이, 화면 내용 등에 따라 용량은 달라집니다.) 결국 저는 GIF 이미지를 얻기 위해 이 방법을 쓰지 않습니다. 다음 글에서는 시뮬레이터로 녹화한 영상을 터미널 앱을 이용해 GIF로 변환하는 방법에 대해서 이야기해 보겠습니다.

## (번외) 터미널 앱으로 시뮬레이터 화면 녹화하기

시뮬레이터에 화면 녹화 기능이 추가되기 전에는 터미널 앱을 이용해 녹화를 하곤 했습니다. 앞으로 터미널 앱으로 시도할 일이 있겠냐마는... 기록용으로 한번 남겨봅니다!
...
3. 터미널에 `Recording started`이 출력되며 녹화가 시작됩니다.
